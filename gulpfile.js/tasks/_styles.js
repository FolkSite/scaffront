...
var buildConfigs = getObject.get(Config, 'tasks.build');
var distConfigs = getObject.get(Config, 'tasks.dist');

var buildTasks = [];
var distTasks = [];

if (buildConfigs) {
  _.each(buildConfigs, function (config, section) {
    if (!config.src || !config.dest) { return; }

    buildTasks = buildTasks.push(section);

    gulp.task([ns, section, 'build'].join(':'), function (cb) {
...
_.each(Config.tasks, function (configs, section) {
  console.log('section, configs', section, configs);

  _.each(configs, function (config, subsection) {

    gulp.task(section +':'+ subsection, (function (section, subsection) {
      return function (cb) {
